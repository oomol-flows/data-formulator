...
class DataLoadAgent(object):
    ...
    def run(self, input_data, n=1):
        print("input data ", input_data["rows"][0])
        # 用表的前几条数据生成一个简短的sample，同时拉出前几列字段和最后几列字段拼一起，中间用... 连接省略大量的数据。不是全部读取
        ...
        for choice in response.choices:
            ...
            self.logger.info("\n=== Data load result ===>\n")
            self.logger.info(choice.message.content + "\n")
            
            json_blocks = extract_json_objects(choice.message.content + "\n")
            self.logger.info(json_blocks)
            
            if len(json_blocks) > 0:
                result: dict[str, Any]  = {'status': 'ok', 'content': json_blocks[0]}
            else:
                try:
